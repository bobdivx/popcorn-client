 ...
 * Fait un proxy vers le backend Rust /api/sync/reset
 */
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    console.log('[SYNC STOP] 🛑 Arrêt de la synchronisation...');
    ...
    const backendApiUrl = `${backendUrl}/api/sync/reset`;
    
    console.log(`[SYNC STOP] 📡 Proxy vers: ${backendApiUrl}`);
    ...
    try {
      ...
      console.log(`[SYNC STOP] ✅ Réponse du backend: ${response.status}`);
    } catch (fetchError) {
      clearTimeout(timeoutId);
      console.error(`[SYNC STOP] ❌ Erreur:`, fetchError);
    ...
    console.error('[SYNC STOP] ❌ Erreur:', error);
    return new Response(
      JSON.stringify({
        success: false,
