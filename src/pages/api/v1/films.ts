...
/**
 * GET /api/v1/films
 * Récupère les films (torrents de catégorie Movies)
 * Fait un proxy vers le backend Rust /api/torrents/list avec category=Movies
 */
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    console.log('[FILMS] 🎬 Récupération des films...');
    
    // Récupérer les paramètres de pagination depuis l'URL
    const page = url.searchParams.get('page') || '1';
    const limit = url.searchParams.get('limit') || '1000'; // Récupérer tous les films pour le tri côté client
    ...
    // Utiliser "films" comme catégorie (comme dans le code de synchronisation)
    const backendApiUrl = `${backendUrl}/api/torrents/list?category=films&page=${page}&limit=${limit}&sort=${sort}`;
    
    console.log(`[FILMS] 📡 Proxy vers: ${backendApiUrl}`);
    ...
    try {
      ...
      console.log(`[FILMS] ✅ Réponse du backend: ${response.status} ${response.statusText}`);
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`[FILMS] ❌ Erreur HTTP ${response.status}:`, errorText);
      }
    } catch (fetchError) {
      clearTimeout(timeoutId);
      console.error(`[FILMS] ❌ Erreur:`, fetchError);
    ...
    if (!response.ok) {
      console.error(`[FILMS] ❌ Erreur HTTP ${response.status}:`, responseBody);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.log(`[FILMS] 📦 Réponse brute du backend:`, {
      success: responseData.success,
      dataLength: Array.isArray(responseData.data) ? responseData.data.length : 0,
      hasData: !!responseData.data,
    });
    
    // Transformer les données pour correspondre au format FilmData
      ...
      const films = responseData.data
        ...
        .map((torrent: any) => {
          ...
          return {
            ...
            releaseDate: releaseDate, // Optionnel maintenant
            genres: genres, // Genres depuis TMDB
          };
        });
      
      console.log(`[FILMS] ✅ ${films.length} film(s) transformé(s)`);
      responseData.data = films;
    } else {
      console.warn(`[FILMS] ⚠️ Pas de données valides dans la réponse:`, {
        success: responseData.success,
        hasData: !!responseData.data,
