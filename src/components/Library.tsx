    ...
    try {
      ...
      void (async () => {
        ...
        const sections = await Promise.all(
          shared.map(async (s) => {
            if (!s.backendUrl || typeof s.backendUrl !== 'string') return null;
            try {
              const baseUrl = s.backendUrl.trim().replace(/\/$/, '');
              new URL(baseUrl);
            ...
            try {
              ...
              const friendBaseUrl = s.backendUrl.replace(/\/$/, '');
              // Requête vers le backend de l'ami : si le domaine ne résout pas (ERR_NAME_NOT_RESOLVED)
              // ou est hors ligne, le navigateur logue l'erreur réseau ; on attrape ici et on ignore
              // pour ne pas bloquer l'affichage de la bibliothèque locale.
              const res = await fetch(`${friendBaseUrl}/library`, {
                method: 'GET',
                headers: {
              ...
              // Domaine non résolu (ERR_NAME_NOT_RESOLVED), timeout, CORS ou backend éteint : normal
              // si l'ami est en local ou son URL n'est pas joignable depuis ici.
              const friendLabel = s.email || s.friendId;
              if (typeof console !== 'undefined' && console.warn) {
                console.warn(
                  '[Popcorn] Backend ami inaccessible:',
                  friendLabel || s.backendUrl,
                  '— URL:',
