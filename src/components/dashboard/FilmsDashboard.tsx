...
export default function FilmsDashboard() {
  ...
  // Lignes "Nouveautés" (par date de sortie TMDB) et "Les plus seedés" pour un rendu type appli de streaming.
  ...
  const mostSeededFilmsRow = useMemo(() => {
    const withSeeds = films.filter((f) => typeof f.seeds === 'number' && (f.seeds ?? 0) > 0);
  ...
  return (
    <div className="min-h-screen bg-page text-white flex flex-col safe-area-x">
      ...
      <div className="pb-8 tv:pb-12 flex-1 safe-area-bottom" style={{ paddingBottom: 'max(2rem, var(--safe-area-inset-bottom))' }}>
        ...
        {/* Section Nouveautés (sorties récentes TMDB) — masquée pendant la sync torrent */}
        {!isSyncing && recentFilmsRow.length >= MIN_ITEMS_PER_GENRE_ROW && (
          <CarouselRow title={t('dashboard.newReleasesMovies')} autoScroll={false}>
            {recentFilmsRow.map((film) => (
              <div
                key={film.id}
        ...
        {/* Section Les plus seedés (torrents avec le plus de seed) */}
        {mostSeededFilmsRow.length >= MIN_ITEMS_PER_GENRE_ROW && (
          <CarouselRow title={t('dashboard.mostShared')} autoScroll={false}>
            {mostSeededFilmsRow.map((film) => (
              <div
                key={film.id}
                className="flex-shrink-0 w-[140px] sm:w-[160px] md:w-[180px] lg:w-[280px] xl:w-[320px] tv:w-[400px]"
              >
