...
export default function BackendDiagnostics() {
  ...
  const run = async () => {
    ...
    try {
      ...
      out.unshift({
        name: language === 'fr' ? 'Résumé' : 'Summary',
        ok: allCriticalOk, // popcorn-web n'est pas critique pour le résumé
        ms: Date.now() - started,
        message: allCriticalOk 
          ? (popcornWebTest && !popcornWebTest.ok 
              ? (language === 'fr' ? 'OK (popcorn-web échoue - vérifiez que popcorn-web est déployé avec CORS configuré)' : 'OK (popcorn-web fails - check that popcorn-web is deployed with CORS configured)')
              : 'OK')
          : (language === 'fr' ? 'Au moins un test critique a échoué' : 'At least one critical test failed'),
      });
  ...
  const copyReport = async () => {
    ...
    try {
      ...
      setCopyMsg(t('backendDiagnostics.copied'));
      return;
    }
    } catch {
      // fallback ci-dessous
    ...
    try {
      ...
      if (ta) {
        ...
        setCopyMsg(t('backendDiagnostics.copyFallback'));
      } else {
        setCopyMsg(t('backendDiagnostics.copyManual'));
      }
    } catch {
      setCopyMsg(t('backendDiagnostics.copyManual'));
    }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="table w-full">
            <thead>
              <tr>
                <th>{t('backendDiagnostics.test')}</th>
                <th>{t('backendDiagnostics.status')}</th>
                <th>{t('backendDiagnostics.duration')}</th>
                <th>{t('backendDiagnostics.detail')}</th>
              </tr>
            </thead>
            <tbody>
              {(rows || []).map((r) => {
                ...
                const isSummary = r.name === (language === 'fr' ? 'Résumé' : 'Summary');
                    ...
                    <td className="font-medium text-white break-words min-w-[150px]">
                      ...
                      {isEmulatorTest && (
                        <span className="ml-2 text-xs text-blue-400/70 italic break-words">
                          ({language === 'fr' ? 'uniquement émulateur Android' : 'Android emulator only'})
                        </span>
                      )}
                    </td>
              ...
              {rows === null && (
                <tr>
                  <td className="text-gray-400" colSpan={4}>{t('backendDiagnostics.loading')}</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h3 className="text-base font-bold text-white">{t('backendDiagnostics.report')}</h3>
          <div className="flex items-center gap-2">
            <button className="btn btn-sm w-full sm:w-auto" onClick={() => copyReport()} disabled={!report}>
              {t('backendDiagnostics.copy')}
            </button>
          </div>
        </div>
        ...
        <textarea
          ...
          readOnly
          value={report || (language === 'fr' ? 'Génération du rapport…' : 'Generating report...')}
        />
        <p className="text-gray-500 text-xs break-words">
          {t('backendDiagnostics.reportTip')}
        </p>
      </div>
    </div>
  );
