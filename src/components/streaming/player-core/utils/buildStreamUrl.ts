...
export function buildStreamUrl({
  ...
  // Ne pas l'utiliser pour les médias bibliothèque (info_hash local_xxx) : le backend ne les accepte pas (400).
  if (
    streamingTorrentMode &&
    infoHash &&
    !infoHash.startsWith('local_') &&
    streamingTorrentToken &&
    (typeof fileIndex === 'number' || fileIndex == null)
  ) {
    ...
    const streamUrlSt = `${baseUrl.replace(/\/$/, '')}/api/stream-torrent/${infoHash}/${idx}/${encName}?${tokenParam}${qualityParam}`;
    return {
