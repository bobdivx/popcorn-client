...
export function SubtitleSelector({
  ...
  const getLanguageName = (lang?: string, name?: string) => {
    if (name) return name;
  ...
  return (
    <div class="absolute inset-0 bg-black/90 z-50 flex flex-col">
      <div class={`flex items-center justify-between ${isTV ? 'p-8' : isFullscreen ? 'p-6' : 'p-4'}`}>
        ...
        >
          <X class={`${isTV ? 'w-6 h-6' : isFullscreen ? 'w-5 h-5' : 'w-4 h-4'} text-white`} />
        </button>
      </div>
      ...
      <div class={`flex-1 overflow-y-auto ${isTV ? 'px-8 pb-8' : isFullscreen ? 'px-6 pb-6' : 'px-4 pb-4'}`}>
        {audioTracks.length > 0 && (
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
              <Languages class={`${isTV ? 'w-6 h-6' : isFullscreen ? 'w-5 h-5' : 'w-4 h-4'} text-white`} />
              <h3 class={`${isTV ? 'text-xl' : isFullscreen ? 'text-lg' : 'text-base'} font-semibold text-white`}>
                Pistes Audio
              </h3>
            </div>
            <div class="space-y-2">
              {audioTracks.map((track) => (
                <button
                  ...
                  class={`w-full text-left ${isTV ? 'p-4' : isFullscreen ? 'p-3' : 'p-2'} rounded-lg transition-all ${
                ...
                >
                  ...
                  {track.default && (
                    <span class={`ml-2 ${isTV ? 'text-sm' : 'text-xs'} opacity-75`}>(Par défaut)</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div>
          <div class="flex items-center gap-3 mb-4">
            <Subtitles class={`${isTV ? 'w-6 h-6' : isFullscreen ? 'w-5 h-5' : 'w-4 h-4'} text-white`} />
            <h3 class={`${isTV ? 'text-xl' : isFullscreen ? 'text-lg' : 'text-base'} font-semibold text-white`}>
              Sous-titres
            </h3>
          </div>
