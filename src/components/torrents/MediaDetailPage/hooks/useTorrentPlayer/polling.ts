  ...
  return async (infoHash: string) => {
    ...
    try {
      ...
      if (stats) {
        ...
        // Important: juste après l'ajout d'un torrent, le client peut répondre 404/null
        // pendant quelques secondes. Dans ce cas, on garde le polling au lieu de stopper.
        const now = Date.now();
        if (notFoundStartTimeRef.current === null) {
          notFoundStartTimeRef.current = now;
        }

        const waitedMs = now - notFoundStartTimeRef.current;
        if (waitedMs >= TORRENT_NOT_FOUND_TIMEOUT_MS) {
          setPlayStatus('error');
          setErrorMessage('Torrent introuvable dans le client');
          addDebugLog('error', '❌ Torrent introuvable après attente', {
            infoHash,
...
async function handleCompletedState(
  ...
  const { setPlayStatus, setProgressMessage, addDebugLog, loadVideoFiles, setVideoFiles, setSelectedFile, setIsAvailableLocally, stopProgressPolling, setCountdownRemaining } = context;
  ...
  try {
    ...
    if (videos.length > 0) {
      // Marquer disponible uniquement quand des fichiers vidéo sont bien présents
      ...
      // Compte à rebours 3s puis lancement automatique (au lieu de lancer immédiatement)
      addDebugLog('success', '✅ Lancement automatique dans 3 secondes…', {
