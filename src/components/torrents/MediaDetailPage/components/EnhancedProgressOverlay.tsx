...
export function EnhancedProgressOverlay({
  ...
  return (
    <div className="player-progress-overlay fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex items-center justify-center">
      {/* Fond avec l''image et effet de profondeur */}
      {imageUrl && (
        <>
          <div
            className="absolute inset-0 bg-cover bg-center opacity-30 scale-105 transition-[opacity,transform] duration-700 blur-xl"
      ...
      {/* Fermer / Annuler — toujours accessible (TV / tactile) */}
      <button
        type="button"
        onClick={() => (confirmingCancel ? setConfirmingCancel(false) : setConfirmingCancel(true))}
        title={t('common.cancel') || 'Annuler'}
        aria-label={t('common.cancel') || 'Annuler'}
        tabIndex={0}
        data-focusable
        className="absolute top-4 left-4 z-40 inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 text-white border border-white/20 focus:outline-none focus:ring-2 focus:ring-primary-400 transition-[opacity,transform,background-color] duration-200 active:scale-95"
      >
        <svg
      ...
      {/* Contenu */}
      <div className="player-overlay-content relative z-10 max-w-lg w-full mx-4">
        {/* Erreur */}
        {playStatus === 'error' && (
        ...
        {playStatus !== 'error' && (
          <div className="flex flex-col items-center">
            ...
            <div className="mt-10 flex flex-col items-center gap-4">
              {confirmingCancel ? (
                <div className="w-full max-w-sm rounded-xl border border-white/15 bg-black/50 px-5 py-4 text-center space-y-3 transition-[opacity,transform] duration-200">
                  ...
                  <div className="flex justify-center gap-3">
                    <button
                      ...
                      data-focusable
                      tabIndex={0}
                      className="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-[opacity,transform,background-color] duration-200 border border-white/20 font-medium active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary-400"
                    >
                    ...
                    <button
                      ...
                      data-focusable
                      tabIndex={0}
                      className="px-5 py-2.5 bg-red-600/80 hover:bg-red-600 text-white rounded-lg transition-[opacity,transform,background-color] duration-200 font-medium active:scale-95 focus:outline-none focus:ring-2 focus:ring-red-400"
                    >
                ...
                <div className="flex gap-4">
                  {onContinueInBackground &&
                    (playStatus === 'downloading' ||
                      ...
                      <button
                        ...
                        data-focusable
                        tabIndex={0}
                        className="px-8 py-3 bg-green-600/80 hover:bg-green-600 text-white rounded-lg transition-[opacity,transform,background-color] duration-200 font-medium shadow-lg shadow-green-600/30 active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary-400"
                      >
                  ...
                  <button
                    ...
                    data-focusable
                    tabIndex={0}
                    className="px-10 py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-[opacity,transform,background-color] duration-200 border border-white/20 hover:border-white/30 font-medium active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary-400"
                  >
                    Annuler
                  </button>
                </div>
              ...
              <button
                ...
                data-focusable
                tabIndex={0}
                className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-xs rounded-lg transition-[opacity,background-color] duration-200 border border-white/10 hover:border-white/20 focus:outline-none focus:ring-2 focus:ring-primary-400"
              >
