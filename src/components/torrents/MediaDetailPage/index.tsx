...
export default function MediaDetailPage({ torrent, initialVariants, seriesEpisodes, initialTorrentStats, backHref, streamBackendUrl }: MediaDetailPageProps) {
  ...
  /** Après ajout d'un seul épisode (streaming), info_hash pour que le lecteur utilise ce torrent */
  ...
  // Pouvoir lancer la lecture si on a un infoHash (stream-torrent ou déjà ajouté) ou un magnet/lien pour ajouter puis stream
  ...
  // Médias bibliothèque (local_xxx) : pas d'API stream-torrent côté backend, toujours utiliser le flux local.
  ...
  /** Lire un seul épisode du pack (preview) en streaming : ajoute avec for_streaming + only_files [fileIndex], puis lance le lecteur. */
  ...
  /** Index du fichier (dans packPreviewFiles) sélectionné pour "Télécharger cet épisode" / "Lire" (preview, torrent pas encore ajouté) */
  ...
  /** Décompose les fichiers du pack en saisons/épisodes via SxxExx ou 1x05 dans les noms */
  ...
  /** Même structure à partir de la liste preview (sans torrent ajouté) : permet d'afficher les épisodes et "Télécharger tout" */
  ...
  /** Afficher la décomposition par saison/épisode soit depuis les fichiers du torrent (après ajout), soit depuis la preview (avant ajout) */
      ...
      <div className="relative z-10" ref={tvBackHandlerRef} data-tv-back-handler>
        <div className="relative w-full min-h-[60vh] sm:min-h-[70vh] flex flex-col justify-end px-3 sm:px-4 md:px-6 lg:px-16 pb-8 sm:pb-12 md:pb-16 pt-20 sm:pt-24 md:pt-32">
          ...
          <div className="max-w-4xl">
            ...
            {/* Séries : section Épisodes type streaming (saisons + liste d’épisodes) */}
            {seriesEpisodes?.seasons?.length ? (
              <div className="mb-6 space-y-6">
                ...
                />
                {/* Pack complet sélectionné : hint, chargement, puis liste des fichiers par SxxExx */}
