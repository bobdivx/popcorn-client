...
async function downloadFromExternalIndexerOnce(options: {
  ...
  // Si pas d'infoHash ni magnet mais URL externe (absolue ou relative) : télécharger via le backend
  // - URL absolue (http/https) : backend utilise le paramètre url
  // - URL relative (ex. HD-F : torrents.php?action=download&id=...) : backend utilise indexerId + relativeUrl
  const hasExternalLink = torrent._externalLink && !torrent._externalLink.startsWith('magnet:');
  const isRelativeLink = hasExternalLink && !torrent._externalLink!.startsWith('http://') && !torrent._externalLink!.startsWith('https://');
  const indexerId = (torrent as any).indexerId || (torrent as any).indexer_id;
  if (hasExternalLink && (! isRelativeLink || indexerId)) {
    return await downloadFromExternalIndexer({
      torrent,
      baseUrl,
      headers,
  ...
  // Si aucun infoHash, magnet ni URL externe (ou lien relatif sans indexerId), erreur
