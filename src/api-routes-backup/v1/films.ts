...
import type { FilmData } from '../../../lib/client/types.js';
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    // Le backend Rust expose /api/torrents/list avec category=films
    const url = `${backendUrl}/api/torrents/list?category=films&sort=popular&limit=200&page=1`;
    ...
    const films: FilmData[] = rows
      .map((raw: any) => {
        ...
        return {
          ...
          year: typeof releaseDate === 'string' ? new Date(releaseDate).getFullYear() : undefined,
    ...
    return new Response(JSON.stringify({ success: true, data: films }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (e) {
