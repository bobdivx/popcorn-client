...
/**
 * Récupère ou met à jour les paramètres de synchronisation
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const response = await fetch(`${serverUrl}/api/sync/settings`, {
      method: 'GET',
      headers,
    });
    ...
    console.error('[SYNC SETTINGS GET] Exception:', error);
    return new Response(
      JSON.stringify({
        success: false,
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    console.error('[SYNC SETTINGS POST] Exception:', error);
    return new Response(
      JSON.stringify({
        success: false,
