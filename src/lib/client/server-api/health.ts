...
export const healthMethods = {
  ...
  async getSetupStatus(this: ServerApiClientAccess): Promise<ApiResponse<SetupStatus>> {
    ...
    // Vérifier si une clé TMDB est configurée (obligatoire pour le wizard → afficher l'étape TMDB si absente)
    let hasTmdbKey = false;
    const userId = this.getCurrentUserId();
    if (userId) {
      const tmdbRes = await this.backendRequest<{ has_key?: boolean }>('/api/tmdb/key', {
        method: 'GET',
