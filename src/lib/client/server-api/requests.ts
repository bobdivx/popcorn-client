...
export const requestsMethods = {
  ...
  async listMediaFavorites(
    ...
    try {
      const res = await this.backendRequest<MediaFavorite[]>(`/api/favorites${query ? '?' + query : ''}`, { method: 'GET', headers });
      if (!res.success) {
  ...
  async addMediaFavorite(
    ...
    const userId = this.getCurrentUserId?.() ?? null;
    const headers: HeadersInit = userId ? { 'X-User-ID': userId } : {};
    const res = await this.backendRequest<MediaFavorite>('/api/favorites', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify(data),
    });
    if (!res.success && (res as { error?: string }).error === 'NotFound') {
      return { success: false, message: 'Favoris non supportés par ce serveur.', error: 'NotFound' };
    }
    return res;
  },
  ...
  async removeMediaFavorite(
    ...
    const userId = this.getCurrentUserId?.() ?? null;
    const headers: HeadersInit = userId ? { 'X-User-ID': userId } : {};
    const res = await this.backendRequest<boolean>(
  ...
  async checkMediaFavorite(
    ...
    const userId = this.getCurrentUserId?.() ?? null;
    const headers: HeadersInit = userId ? { 'X-User-ID': userId } : {};
    const res = await this.backendRequest<{ is_favorite: boolean }>(
