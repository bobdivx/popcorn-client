...
export const dashboardMethods = {
  ...
  async getFilmsData(this: ServerApiClientDashboardAccess, language?: string): Promise<ApiResponse<FilmData[]>> {
    ...
    const lang = toTmdbLanguage(language);
    const res = await this.backendRequest<any[]>(`/api/torrents/list?category=films&sort=release_date&limit=30&page=1&skip_indexer=true&lang=${lang}`, {
      method: 'GET',
    });
    if (!res.success) {
    ...
    const films: FilmData[] = rows
      .map((raw: any, index: number) => {
        ...
        return {
          ...
          ...(typeof raw?.tmdbTitle === 'string' && raw.tmdbTitle.trim()
            ? { tmdbTitle: raw.tmdbTitle.trim() }
            : typeof raw?.tmdb_title === 'string' && raw.tmdb_title.trim()
              ? { tmdbTitle: raw.tmdb_title.trim() }
              : {}),
          type: 'movie',
  ...
  async getSeriesData(this: ServerApiClientDashboardAccess, language?: string): Promise<ApiResponse<SeriesData[]>> {
    ...
    // skip_indexer=true pour éviter les requêtes lentes à l'indexer
    const lang = toTmdbLanguage(language);
    const res = await this.backendRequest<any[]>(`/api/torrents/list?category=series&sort=release_date&limit=30&page=1&skip_indexer=true&lang=${lang}`, {
      method: 'GET',
    });
    if (!res.success) {
