...
/** True si l'URL backend courante est celle d'un ami (pas « mon serveur »). */
function isFriendBackend(): boolean {
  const myUrl = getMyBackendUrl();
  const current = getBackendUrlSafe();
  return myUrl != null && current != null && current !== myUrl;
}
...
function registerConnectionFailureListener() {
  ...
  try {
    serverApi.addConnectionFailureListener(() => {
      // Ne jamais marquer « offline » quand c’est le serveur d’un ami qui échoue (séparation fiable).
 ...
 * Ne met jamais le store en « offline » quand c'est le backend d'un ami qui échoue.
 */
export async function checkBackendConnection(): Promise<boolean> {
  registerConnectionFailureListener();

  if (isFriendBackend()) {
    // Ne pas toucher au store global : le backend d'un ami est complètement séparé.
    try {
      const res = await serverApi.checkServerHealth();
      return res.success && res.data?.reachable !== false;
    } catch {
      return false;
    }
  }
