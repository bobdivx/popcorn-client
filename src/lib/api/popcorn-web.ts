...
/**
 * Récupère les définitions d'indexers en passant par le backend (proxy) si disponible,
 * sinon via l'appel direct à popcorn-web. À utiliser pour l'ajout d'indexer afin d'éviter
 * CORS et timeouts (liste complète, recherche).
 * @param backendBaseUrl URL de base du backend (ex. http://localhost:3000) ou null pour ignorer le proxy
 * @returns Liste des définitions ou null
 */
export async function getIndexerDefinitionsWithBackendFallback(
  backendBaseUrl: string | null
): Promise<IndexerDefinition[] | null> {
  const base = backendBaseUrl?.trim().replace(/\/$/, '');
  if (!base) {
    return getIndexerDefinitions();
  }
  try {
    const url = `${base}/api/client/indexer-definitions`;
    const controller = new AbortController();
    ...
    if (response.ok && data.success && data.data && Array.isArray(data.data.definitions)) {
      return data.data.definitions;
    }
  } catch {
    // Backend injoignable ou erreur : on continue avec l'appel direct
  }
  return getIndexerDefinitions();
}
...
/**
 * Envoie les captures d'écran (générées par le backend) vers popcorn-web pour hébergement public.
 * Les URLs dans la description C411 pourront ainsi pointer vers https://popcornn.app/... et s'afficher sur le tracker.
 * @param mediaId — identifiant du média local (UUID)
 * @param backendBaseUrl — URL de base du backend (ex. http://localhost:3000) pour récupérer les images
 * @param count — nombre de captures (0, 1, 2 ou 3)
 * @returns success et message d'erreur éventuel
 */
export async function uploadScreenshotsToCloud(
  mediaId: string,
  ...
  // Envoyer en JSON (base64) au lieu de multipart pour éviter le blocage
  // « Cross-site POST form submissions are forbidden » (Astro CSRF / navigateur).
  // Le backend convertit en multipart avant d'appeler popcorn-web.
  const apiUrl = `${base}/api/client/uploader/screenshots-to-cloud`;

  // Redimensionne une capture en JPEG allégé côté navigateur pour éviter les erreurs 413.
  async function downscaleToJpeg(blob: Blob, maxWidth = 1280): Promise<Blob> {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      const imgUrl = `${base}/api/library/uploader/screenshots/${encodeURIComponent(mediaId)}/${i}.png`;
      const res = await fetch(imgUrl, { method: 'GET' });
      if (!res.ok) {
        return { success: false, message: `Impossible de récupérer la capture ${i + 1} depuis le backend` };
      }
      const originalBlob = await res.blob();
      const jpegBlob = await downscaleToJpeg(originalBlob, 1280);
      filesBase64.push(await blobToBase64(jpegBlob));
    }
    const body = JSON.stringify({ media_id: mediaId, files: filesBase64 });
    const res = await requestWithTokenRefresh(
      apiUrl,
      {
        method: 'POST',
    ...
    if (!res.ok) {
      const msg = res.data?.message || res.data?.error || `Erreur ${res.status}`;
      console.warn('[POPCORN-WEB] uploadScreenshotsToCloud:', msg);
      return { success: false, message: msg };
    }
    return { success: true };
  } catch (e) {
    console.warn('[POPCORN-WEB] uploadScreenshotsToCloud réseau/exception:', e);
    return { success: false, message: e instanceof Error ? e.message : 'Erreur réseau' };
  }
}
