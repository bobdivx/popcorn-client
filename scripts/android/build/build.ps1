...
# Usage: .\scripts\android\build\build.ps1 [-Demo] [mobile|tv|standard] [aab]
...
# Les arguments peuvent être dans n'importe quel ordre (ignorer -Demo pour allArgs)
$allArgs = @($Variant, $BuildType) + $args | Where-Object { $_ -ne "" -and $_ -ne $null -and $_ -ne "-Demo" }
...
# Compatibilité Android TV (aligné avec le workflow CI) : banner TV + patch manifest (Mobile + TV)
$manifestPath = Join-Path $script:ProjectRoot "src-tauri\gen\android\app\src\main\AndroidManifest.xml"
if (Test-Path $manifestPath) {
    Write-Info "Compatibilité Android TV : création du banner et patch du manifest..."
    Push-Location $script:ProjectRoot
    npm run create:tv-banner 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) { Write-Ok "Banner TV créé" } else { Write-Warn "create:tv-banner non appliqué (optionnel)" }
    npm run patch:android 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) { Write-Ok "Patch AndroidManifest (Mobile + TV) appliqué" } else { Write-Warn "patch:android non appliqué" }
    Pop-Location
} else {
    Write-Info "AndroidManifest non encore généré, create:tv-banner et patch:android seront à lancer après le premier build si besoin."
}
...
if ($buildAab) {
    $buildCommand = switch ($buildType) {
        "mobile" { "npx cross-env $demoEnv$versionEnv$codeEnv$variantEnv npm run tauri:build:android-mobile:aab" }
        "tv" { "npx cross-env $demoEnv$versionEnv$codeEnv$variantEnv npm run tauri:build:android-tv:aab" }
        default { Write-Err "AAB non supporté pour la variante 'standard'. Utilisez 'mobile' ou 'tv'."; exit 1 }
    }
} else {
    $buildCommand = switch ($buildType) {
        "mobile" { "npx cross-env $demoEnv$versionEnv$codeEnv$variantEnv npm run tauri:build:android-mobile" }
        "tv" { "npx cross-env $demoEnv$versionEnv$codeEnv$variantEnv npm run tauri:build:android-tv" }
        default { "npx cross-env $demoEnv$versionEnv$codeEnv$variantEnv npm run tauri:build:android" }
    }
}
...
try {
    ...
    # Erreur déjà traitée dans les blocs internes
} finally {
    Pop-Location
}