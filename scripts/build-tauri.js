...
// Charger les variables d'environnement depuis .env (optionnel, ex. CI sans dotenv installé)
try {
  const { config } = await import('dotenv');
  config();
} catch {
  // dotenv non disponible ou .env absent, continuer sans
}
...
(async () => {
  try {
    ...
    // Détecter la plateforme (desktop / android / ios)
    ...
    try {
      ...
      if (buildEnv.PUBLIC_DEMO_BACKEND_URL) {
        console.log(`ℹ️  PUBLIC_DEMO_BACKEND_URL: ${buildEnv.PUBLIC_DEMO_BACKEND_URL}`);
      }
