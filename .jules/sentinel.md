...
## 2026-06-09 - Prevent Sensitive Key Exposure in UI
**Vulnerability:** Sensitive API keys (TMDB, Indexer APIs) and Passkeys were rendered using `type="text"` in UI forms, potentially exposing them via browser autofill extensions, shoulder surfing, or screen sharing.
**Learning:** React/Preact UI components dealing with third-party integration secrets must obscure inputs securely by default, recognizing that physical and localized exposures are credible threat vectors for desktop/web client environments.
**Prevention:** Always use `<input type="password" autoComplete="off" />` for sensitive credential inputs in Settings or Setup panels, regardless of whether they are initially populated.
