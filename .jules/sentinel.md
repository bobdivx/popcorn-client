...
## 2025-02-28 - Missing autoComplete on Password Fields
**Vulnerability:** API keys and sensitive tokens in configuration forms lacked `autoComplete="new-password"` on `<input type="password">`.
**Learning:** Browsers and password managers may attempt to autofill or incorrectly capture sensitive inputs like API keys if they are just marked as type="password" without explicit autocomplete disabled.
**Prevention:** Always use `autoComplete="new-password"` on password inputs used for tokens/keys rather than actual user credentials.
