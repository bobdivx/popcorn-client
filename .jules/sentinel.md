...
## 2024-06-11 - Sensitive Input Masking
**Vulnerability:** Several settings components exposed sensitive credentials (e.g., API keys, passkeys, Webhook URLs) via `<input type="text">` fields without disabling browser autocomplete, risking physical shoulder-surfing and browser history leakage.
**Learning:** React/Preact inputs handling sensitive configuration should always use `type="password"` and `autoComplete="off"` to prevent unintentional exposure.
**Prevention:** Always verify input types when creating forms handling credentials, keys, or private URLs.
