...
## 2025-02-14 - Fix plain-text credential exposure in UI inputs
**Vulnerability:** Sensitive credentials like TMDB API keys and Tracker Passkeys were rendered using `<input type="text">` elements in the UI, exposing the keys in plain text to shoulder-surfing, screen-sharing, or video recordings.
**Learning:** Hardcoded text inputs for sensitive secrets create unnecessary physical security exposure.
**Prevention:** Always use `<input type="password">` along with `autoComplete="off"` when displaying or receiving API keys, passkeys, tokens, and other credentials in frontend forms.
